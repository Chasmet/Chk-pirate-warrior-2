...
func _rebuild(island_id: int, serial: int) -> void:
    ...
    if serial != _serial:
        return
    var island_root := _active_island_root()
    if island_root == null:
        return
    var previous := island_root.get_node_or_null("DecorGLBV3")
    if previous != null:
        previous.queue_free()
...
func _build_port_landmark(parent: Node3D, info: Dictionary, island_id: int) -> void:
    ...
    if island_id != 8 and island_id != 9 and island_id != 11:
        _spawn_glb(parent, PALMS[0], _ground(info, -25.0, port_z - 8.0), 6.5, 0.15, false)
        _spawn_glb(parent, PALMS[1], _ground(info, 25.0, port_z - 5.0), 5.2, -0.20, false)
    ...
    var tower_z := size.y * 0.30
    var tower_x := -34.0 if island_id % 2 == 0 else 34.0
...
func _scatter_theme(parent: Node3D, info: Dictionary, island_id: int) -> void:
    ...
    var foliage_allowed := island_id not in [8, 9, 11]
    if foliage_allowed:
        for i in range(10):
            if budget >= MAX_DECOR_PER_ISLAND:
                break
            var x := rng.randf_range(-size.x * 0.37, size.x * 0.37)
            var z := rng.randf_range(-size.y * 0.32, size.y * 0.26)
            if absf(x) < 18.0 and z > size.y * 0.18:
                x += 28.0 if x >= 0.0 else -28.0
            var path := PALMS[i % PALMS.size()] if i < 6 else DECOR + "plante_tropicale.glb"
            var target := rng.randf_range(4.4, 7.0) if i < 6 else rng.randf_range(1.2, 2.0)
            _spawn_glb(parent, path, _ground(info, x, z), target, rng.randf_range(0.0, TAU), false)
            budget += 1

    var rock_count := 15 if island_id in [8, 9, 10, 11] else 10
    for i in range(rock_count):
        if budget >= MAX_DECOR_PER_ISLAND:
            break
        var angle := rng.randf_range(0.0, TAU)
        var rx := rng.randf_range(size.x * 0.20, size.x * 0.40)
        var rz := rng.randf_range(size.y * 0.18, size.y * 0.38)
        var x := cos(angle) * rx
        var z := sin(angle) * rz
        var target := rng.randf_range(2.2, 5.0)
        _spawn_glb(parent, ROCKS[i % ROCKS.size()], _ground(info, x, z), target, rng.randf_range(0.0, TAU), true)
        budget += 1

    var extra := _theme_props(island_id)
    for i in range(extra.size()):
        if budget >= MAX_DECOR_PER_ISLAND:
            break
        var path := str(extra[i])
        var side := -1.0 if i % 2 == 0 else 1.0
        var x := side * rng.randf_range(size.x * 0.10, size.x * 0.30)
        var z := rng.randf_range(-size.y * 0.25, size.y * 0.18)
        var target := 2.7
