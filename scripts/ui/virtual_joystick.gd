...
func _notification(what: int) -> void:
    # Android peut couper un contact sans envoyer le relâchement classique
    # (changement d'application, verrouillage, appel ou perte de focus).
    # Remettre l'entrée à zéro évite un héros qui continue à marcher seul.
...
func _process(_delta: float) -> void:
    # Si un HUD est masqué pendant qu'un doigt est posé, Android peut ne jamais
    # envoyer le relâchement. On neutralise alors aussi le vecteur mémorisé.
