...
func _new_music_player(node_name: String) -> AudioStreamPlayer:
    ...
    player.volume_db = NORMAL_MUSIC_DB
    add_child(player)
    return player
...
func _crossfade_to(path: String) -> void:
    ...
    if not music_player.playing or music_player.stream == null:
        ...
        music_player.volume_db = NORMAL_MUSIC_DB
        music_player.play()
    ...
    _fade_tween.tween_property(transition_player, "volume_db", NORMAL_MUSIC_DB, MUSIC_FADE_SECONDS)
...
func _finish_crossfade(path: String) -> void:
    ...
    transition_player.volume_db = NORMAL_MUSIC_DB
    _current_music_path = path
...
func _update_voice_ducking(delta: float) -> void:
    ...
    var target := VOICE_DUCK_DB if voice_playing else NORMAL_MUSIC_DB
    var speed := 22.0 if voice_playing else 10.0
    music_player.volume_db = move_toward(music_player.volume_db, target, speed * delta)
