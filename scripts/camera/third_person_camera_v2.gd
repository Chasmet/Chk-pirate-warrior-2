...
func _apply_dynamic_camera(immediate: bool, delta: float = 0.016) -> void:
    if _target == null or not is_instance_valid(_target):
        return
    var active_controller := get_tree().get_first_node_in_group("active_controller")
    var boat_mode := active_controller is BoatController and active_controller.is_boarded()
